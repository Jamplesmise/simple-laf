...
>
> **v1.4 更新**:
> - 新增 5.6.6 上下文精准控制与可视化管理 - 规避 Claude Code 上下文不透明问题
>   - 70% 阈值蓝色链接提示，点击查看上下文详情
>   - 分类展示: 系统提示词/对话历史/函数代码/工具结果
>   - 手动删除 + 手动压缩 + 智能压缩
>   - 精准更新操作: 只加载修改点 ±N 行，节省 90% Token
> - 新增 5.6.7 Plan 模式 - 大任务先规划后执行
>   - 只读分析 → 生成计划 → 用户确认 → 选择性执行
>   - 支持暂停/恢复/停止，实时进度展示
>
> **v1.3 更新**:
> - 新增 5.6 Claude Code 技术借鉴 - 参考 Anthropic 官方 CLI 架构优化后台执行
>   - Master Agent Loop（主代理循环）- 单线程主循环 + 受控子代理分发
>   - Hook 系统 - PreToolUse/PostToolUse 生命周期钩子
>   - Subagent 并行执行 - 独立上下文窗口、单一职责
>   - 后台任务持久化 - 长时间任务异步执行
>   - 权限分级模型 - Auto/Confirm/Restricted 三级权限
>
> **v1.2 更新**:
...
### 5.6 Claude Code 技术借鉴

> 参考：Claude Code CLI 架构和 Claude Agent SDK

Claude Code 作为 Anthropic 官方的 CLI 工具，其技术架构对优化 AI Chat 后台执行有重要参考价值。

#### 5.6.1 Master Agent Loop（主代理循环）

Claude Code 采用单线程主循环 + 受控子代理分发的架构：
...
│  │  while (tool_call) {                                    │ │
│  │    1. 执行工具 (execute_tool)                           │ │
│  │    2. 将结果反馈给模型 (feed_results_to_model)          │ │
│  │    3. 获取下一步指令 (get_next_action)                  │ │
│  │  }                                                      │ │
│  │  // 循环终止条件: 模型返回纯文本响应，无工具调用          │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
│  优势:                                                        │
│  • 单线程保持可调试性和可靠性                                  │
...
interface AgentLoop {
  conversationId: string
  status: 'idle' | 'running' | 'waiting_confirm' | 'completed'
  currentStep: number
  totalSteps: number
...
async function runAgentLoop(input: string): Promise<void> {
  ...
  while (hasToolCalls(response)) {
    ...
    const toolResults = await Promise.all(
      response.toolCalls
        ...
        .map(executeToolWithHooks)
    )

    // 反馈结果，获取下一步
    response = await callModel(formatToolResults(toolResults))

    // 发送状态更新到前端
    emitStatusUpdate({ type: 'executing', step: currentStep })
  }

  emitStatusUpdate({ type: 'completed' })
}
```

#### 5.6.2 Hook 系统（生命周期钩子）

Hook 系统允许在工具执行的各个阶段插入自定义逻辑：
...
│  • Stop / SubagentStop - 停止操作拦截                          │
│  • Notification - 需要用户输入时通知                           │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
interface Hook {
  ...
  function?: string          // 内置函数名
  webhook?: string           // 外部 URL
  timeout?: number           // 超时时间
}

// PreToolUse Hook 示例：代码安全检查
...
// 在创建/更新函数前检查代码安全性
async function checkCodeSecurity(toolCall: ToolCall): Promise<HookResult> {
  if (toolCall.name === 'create_function' || toolCall.name === 'update_function') {
    const code = toolCall.params.code
    const issues = await scanCodeForSecurityIssues(code)

    if (issues.critical.length > 0) {
      return {
        action: 'block',
        reason: `发现安全问题: ${issues.critical.join(', ')}`
      }
    }
    ...
    if (issues.warnings.length > 0) {
      return {
        action: 'modify',
        params: { ...toolCall.params, securityWarnings: issues.warnings }
      }
    }
  }
...
const subagents: Subagent[] = [
  {
    id: 'code-analyzer',
    name: '代码分析师',
    description: '分析代码结构、复杂度和质量',
    systemPrompt: '你是代码分析专家...',
    tools: ['explain_code', 'analyze_refactor', 'analyze_code_quality'],
    maxTokens: 8000
  },
  {
    id: 'test-generator',
    name: '测试生成器',
    description: '生成测试用例和边界条件',
    systemPrompt: '你是测试专家...',
    tools: ['debug_function', 'run_function'],
    maxTokens: 8000
  },
  {
    id: 'doc-writer',
...
async function executeSubagentsInParallel(
  ...
  const results = await Promise.all(
    agents.map(agentId => {
      ...
      return executeSubagent(agent, task)
    })
  )
...
// 使用示例：全面代码审查
async function comprehensiveCodeReview(functionId: string) {
  // 并行执行三个子代理
  const results = await executeSubagentsInParallel(
    `审查函数 ${functionId}`,
    ['code-analyzer', 'test-generator', 'doc-writer']
  )
...
│  状态追踪:                                                    │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │ Task #1: 🔄 运行中 (45%) - 正在分析 src/components/...   │ │
│  │ Task #2: ✅ 完成 - 生成了 12 个测试用例                  │ │
│  │ Task #3: ⏸️ 等待确认 - 需要确认删除 3 个文件             │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
interface BackgroundTask {
  id: string
  conversationId: string
  type: 'refactor' | 'test' | 'analyze' | 'build'
  status: 'pending' | 'running' | 'paused' | 'waiting_confirm' | 'completed' | 'failed'
  progress: number
  currentStep: string
  startedAt: Date
  completedAt?: Date
...
class BackgroundTaskManager {
  ...
  // 启动后台任务
  async start(task: Omit<BackgroundTask, 'id' | 'status' | 'progress'>): Promise<string> {
    const id = generateId()
    this.tasks.set(id, { ...task, id, status: 'running', progress: 0 })

    // 异步执行，不阻塞
    this.executeInBackground(id)
...
io.on('connection', (socket) => {
  socket.on('subscribeTask', (taskId) => {
    // 订阅任务状态更新
    taskManager.onProgress(taskId, (progress) => {
      socket.emit('taskProgress', { taskId, ...progress })
    })
  })
})
```
...
│                                                               │
│  权限模式:                                                    │
│  • plan - 只读/规划模式，不执行任何写操作                      │
│  • normal - 正常模式，低风险自动执行                          │
│  • autonomous - 自主模式，大部分操作自动执行                   │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
async function checkPermission(
  ...
  // 检查是否被阻止
  if (config.blocked.includes(tool)) {
    return { allowed: false, reason: '该操作已被禁止' }
  }

  // 检查是否需要确认
  if (config.requireConfirmation.includes(tool)) {
    return {
      allowed: true,
      requireConfirmation: true,
      confirmMessage: generateConfirmMessage(tool, params)
    }
  }

  // 检查是否在允许列表
  if (config.allowedTools.includes('*') || config.allowedTools.includes(tool)) {
    return { allowed: true }
  }

  return { allowed: false, reason: '该工具未被授权' }
}
```

#### 5.6.6 上下文精准控制与可视化管理

> **核心理念**：上下文是 AI 的"工作记忆"，用户应该能**看到**、**理解**、**控制**它
...
│  │                                                         │ │
│  │  💬 对话历史                            12,350 tokens    │ │
│  │     ├─ □ [消息 1] 用户: 帮我创建...      320 tokens  [✕] │ │
│  │     ├─ □ [消息 2] AI: 好的，我来...    1,200 tokens  [✕] │ │
│  │     ├─ ☑ [消息 3] 用户: 修改验证...      180 tokens  [✕] │ │
│  │     └─ ... (展开更多)                                    │ │
│  │                                                         │ │
│  │  📄 引用的函数代码                       8,500 tokens    │ │
│  │     ├─ □ getUserById.ts (全量)         2,100 tokens [✕] │ │
│  │     ├─ ☑ validateUser.ts (全量)        1,800 tokens [✕] │ │
│  │     └─ □ userService.ts (全量)         3,650 tokens [✕] │ │
│  │                                                         │ │
│  │  🔧 工具调用结果                         5,500 tokens    │ │
│  │     ├─ □ run_function 结果 x3          2,100 tokens [✕] │ │
│  │     └─ □ analyze_code 结果             3,400 tokens [✕] │ │
│  │                                                         │ │
│  │  ─────────────────────────────────────────────────────  │ │
│  │  [全选可删除] [删除选中] [压缩选中] [🔮 智能压缩]          │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
│  ✅ 精准更新方式:                                              │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │  只加载第 10-20 行 (修改点 ± 5 行)                       │ │
│  │  Token 消耗: 300 tokens (节省 90%)                       │ │
│  │  上下文污染: 低                                          │ │
│  │                                                         │ │
│  │  // 加载的上下文:                                        │ │
│  │  10│   const user = await db.findById(id)               │ │
│  │  11│   if (!user) {                                     │ │
│  │  12│     throw new Error('User not found')              │ │
│  │  13│   }                                                │ │
│  │  14│                                                    │ │
│  │  15│   if (user.id === undefined) {  // ← 修改点        │ │
│  │  16│     return null                                    │ │
│  │  17│   }                                                │ │
│  │  18│                                                    │ │
│  │  19│   return sanitizeUser(user)                        │ │
│  │  20│ }                                                  │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
│  生成精准 Diff:                                               │
...
│  │  + if (user.id == null) {  // 同时检查 null 和 undefined │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```
...
```typescript
// 更新操作的上下文策略
interface UpdateContextStrategy {
  // 加载模式
  mode: 'full' | 'partial' | 'diff-only'

  // partial 模式配置
  partial: {
    contextLines: number      // 修改点前后加载行数，默认 10
    expandOnDependency: boolean  // 遇到依赖自动扩展
  }

  // 上下文复用
  reuse: {
    enabled: boolean          // 复用未变更的上下文
    hashCheck: boolean        // 通过 hash 检测变更
  }
}
...
interface PreciseUpdateRequest {
  ...
  // 修改类型判断
  changeType: 'minor' | 'moderate' | 'refactor'
}

// AI 自动判断修改规模
function determineChangeType(userRequest: string): 'minor' | 'moderate' | 'refactor' {
  const patterns = {
    minor: /修改|改成|换成|改一下|第\d+行/,
    moderate: /添加|新增|加一个|优化|修复|增加/,
    refactor: /重构|重写|拆分|合并|重新设计/
  }

  for (const [type, pattern] of Object.entries(patterns)) {
    if (pattern.test(userRequest)) return type as any
  }
  return 'moderate'
}
```
...
interface ContextItem {
  id: string
  type: 'system' | 'message' | 'code' | 'code_partial' | 'tool_result'
  source: string              // 来源 (函数名、消息ID)
  content: string
  tokens: number
  timestamp: number
...
class ContextManager {
  ...
  private maxTokens = 40000
  private warningThreshold = 0.7  // 70%

  // 获取使用情况
  getUsage() {
    const used = this.items.reduce((s, i) => s + i.tokens, 0)
    return { used, total: this.maxTokens, percentage: used / this.maxTokens }
  }

  // 是否需要警告
  shouldWarn() {
    return this.getUsage().percentage >= this.warningThreshold
  }

  // 获取分类视图
  getGroupedView() {
    return {
      system: this.items.filter(i => i.type === 'system'),
      messages: this.items.filter(i => i.type === 'message'),
      code: this.items.filter(i => i.type === 'code' || i.type === 'code_partial'),
      toolResults: this.items.filter(i => i.type === 'tool_result')
    }
  }

  // 手动删除
  remove(ids: string[]) {
    this.items = this.items.filter(i => !ids.includes(i.id) || !i.removable)
  }

  // 智能压缩 - 目标压缩到 50%
  async smartCompress(targetPercentage = 0.5): Promise<CompressionResult> {
    const strategies = [
      // 1. 压缩工具结果（保留摘要）
      () => this.compressType('tool_result', 0.1),
      // 2. 压缩早期对话
      () => this.compressOldMessages(0.3),
      // 3. 代码转为骨架
      () => this.compressCodeToSkeleton(),
      // 4. 删除低重要性内容
      () => this.removeLowImportance()
    ]

    const actions: string[] = []
    for (const strategy of strategies) {
      if (this.getUsage().percentage <= targetPercentage) break
      const action = await strategy()
      actions.push(action)
    }

    return { success: true, actions, newPercentage: this.getUsage().percentage }
  }

  // 手动压缩选中项
  async compressSelected(ids: string[]) {
    for (const id of ids) {
      const item = this.items.find(i => i.id === id)
      if (item?.compressible) {
        item.content = await this.summarize(item.content, item.type)
...
##### 5.6.6.3 与 Claude Code 对比

| 特性 | Claude Code | Simple IDE (改进) |
|------|-------------|-------------------|
| 上下文可见性 | ❌ 不可见 | ✅ 完整可视化面板 |
| 使用量提示 | ⚠️ 模糊 | ✅ 精确百分比 + 分类统计 |
| 70% 阈值提醒 | ❌ 无 | ✅ 蓝色链接提示 |
| 手动删除 | ❌ 无 | ✅ 勾选删除 |
| 手动压缩 | ❌ 无 | ✅ 选择性压缩 |
| 智能压缩 | ⚠️ 自动不可控 | ✅ 一键智能 + 预览 |
| 更新操作 | ⚠️ 全量加载 | ✅ 精准加载修改点 |
| 上下文复用 | ❌ 无 | ✅ 未变更内容复用 |
...
Claude Code 的 Plan 模式是一个重要特性，让 AI 在执行前先规划，用户确认后再执行。
...
│  │                                                         │ │
│  │ ### 分析结果                                             │ │
│  │ - 当前 authMiddleware.ts 包含 450 行代码                 │ │
│  │ - 职责混杂: 认证、授权、会话管理、Token 刷新              │ │
│  │ - 建议拆分为 4 个独立模块                                │ │
│  │                                                         │ │
│  │ ### 执行步骤                                             │ │
│  │ □ Step 1: 创建 services/auth/ 目录                      │ │
│  │ □ Step 2: 提取 TokenService (Token 管理)                │ │
│  │ □ Step 3: 提取 SessionService (会话管理)                │ │
│  │ □ Step 4: 提取 AuthorizationService (权限检查)          │ │
│  │ □ Step 5: 重构 authMiddleware.ts (只保留入口逻辑)        │ │
...
│  │                                                         │ │
│  │ ✅ Step 1: 创建目录 ............................ 完成   │ │
│  │ ✅ Step 2: 提取 TokenService .................. 完成   │ │
│  │ 🔄 Step 3: 提取 SessionService ................ 进行中  │ │
│  │ ⏳ Step 4: 提取 AuthorizationService .......... 等待   │ │
│  │ ⏳ Step 5: 重构 authMiddleware ................. 等待   │ │
│  │                                                         │ │
│  │ [⏸️ 暂停] [⏹️ 停止] [📋 查看详情]                         │ │
│  └─────────────────────────────────────────────────────────┘ │
│                                                               │
...
```typescript
// 计划模式状态
type PlanMode = 'off' | 'planning' | 'reviewing' | 'executing' | 'paused'
...
interface PlanStep {
  ...
  title: string
  description: string
  type: 'create' | 'update' | 'delete' | 'move' | 'test' | 'other'

  // 执行状态
  status: 'pending' | 'running' | 'completed' | 'failed' | 'skipped'
  selected: boolean           // 用户是否选择执行

  // 执行详情
  details?: {
    targetFile?: string
    changes?: string          // 预览的变更
    result?: string           // 执行结果
    error?: string            // 错误信息
  }
...
class PlanModeManager {
  private mode: PlanMode = 'off'
  private currentPlan: ExecutionPlan | null = null

  // 进入计划模式
  async enterPlanMode(userRequest: string): Promise<ExecutionPlan> {
    this.mode = 'planning'
  ...
  private async generatePlan(request: string): Promise<ExecutionPlan> {
    // 调用 AI，但限制只能使用只读工具
    const readOnlyTools = ['explain_code', 'analyze_refactor', 'search_code', 'get_file_tree']

    const analysis = await callAIWithTools(request, readOnlyTools, {
      systemPrompt: `你现在处于计划模式。分析用户需求，生成详细的执行计划。
      不要执行任何写操作，只进行分析和规划。
      输出格式：JSON { analysis, steps, impact }`
    })
  ...
  // 用户修改计划
  updatePlan(updates: Partial<ExecutionPlan>) {
    if (this.currentPlan) {
  ...
  // 选择/取消步骤
  toggleStep(stepId: string, selected: boolean) {
    const step = this.currentPlan?.steps.find(s => s.id === stepId)
    if (step) {
      step.selected = selected
      // 如果取消选择，也取消依赖此步骤的后续步骤
      if (!selected) {
        this.cascadeDeselect(stepId)
      }
    }
  }

  // 开始执行
  async execute(selectedStepIds?: string[]): Promise<void> {
    if (!this.currentPlan) return

    this.mode = 'executing'

    const stepsToExecute = selectedStepIds
      ? this.currentPlan.steps.filter(s => selectedStepIds.includes(s.id))
      : this.currentPlan.steps.filter(s => s.selected)
    ...
    for (const step of stepsToExecute) {
      ...
      try {
        ...
        // 询问用户是否继续
        const shouldContinue = await this.askUserToContinue(step, error)
...
// 判断是否应该进入 Plan 模式
function shouldEnterPlanMode(request: string, estimatedImpact: any): boolean {
  // 关键词匹配
  if (planModeTriggers.keywords.some(kw => request.includes(kw))) {
    return true
  }

  // 影响范围检查
  if (estimatedImpact.filesAffected >= planModeTriggers.impactThresholds.filesAffected) {
    return true
  }

  // 用户设置始终使用 Plan 模式
  if (userSettings.alwaysUsePlanMode) {
    return true
  }
...
```typescript
// Plan 模式对话框组件
function PlanModeDialog({ plan, onExecute, onModify, onCancel }: {
  plan: ExecutionPlan
  ...
  return (
    <div className="plan-mode-dialog">
      <div className="plan-header">
        <h2>🎯 执行计划: {plan.title}</h2>
      ...
      <div className="plan-analysis">
        <h3>📊 分析结果</h3>
        <p>{plan.analysis.currentState}</p>
        {plan.analysis.issues.length > 0 && (
          <div className="issues">
            <h4>发现的问题:</h4>
            <ul>{plan.analysis.issues.map(i => <li key={i}>{i}</li>)}</ul>
          </div>
        )}
      </div>
      ...
      <div className="plan-impact">
        ...
        <div className="impact-stats">
          <span>新增: {plan.impact.newFiles.length} 文件</span>
          <span>修改: {plan.impact.modifiedFiles.length} 文件</span>
          <span>删除: {plan.impact.deletedFiles.length} 文件</span>
          <span className={`risk-${plan.impact.riskLevel}`}>
            风险: {plan.impact.riskLevel}
          </span>
        </div>
      </div>
      ...
      <div className="plan-actions">
        <button onClick={() => onExecute(selectedSteps)} className="primary">
          ✅ 执行选中 ({selectedSteps.length}/{plan.steps.length})
        </button>
        <button onClick={() => onModify(plan)}>
          📝 修改计划
        </button>
...
##### 5.6.7.4 Plan 模式与普通模式对比

| 特性 | 普通模式 | Plan 模式 |
|------|----------|-----------|
| 执行方式 | 立即执行 | 先规划后执行 |
| 用户控制 | 逐步确认 | 整体确认 + 选择性执行 |
| 适用场景 | 小改动 | 大规模重构 |
| 风险可控性 | 中 | 高 |
| Token 效率 | 可能重复加载 | 一次分析，按需执行 |
| 可回退性 | 需要手动 | 自动记录检查点 |
...
将 Claude Code 技术整合到 Simple IDE 的推荐顺序：

| 阶段 | 技术 | Sprint | 价值 |
|------|------|--------|------|
| 1 | Agent Loop + 状态可视化 | 10 | 用户了解 AI 正在做什么 |
| 2 | Hook 系统（PreToolUse） | 10 | 操作拦截和参数校正 |
| 3 | **上下文可视化管理** | 10 | 用户掌控上下文 |
| 4 | **精准更新操作** | 10 | 高效代码迭代 |
| 5 | **Plan 模式** | 10 | 大任务规划执行 |
| 6 | 后台任务持久化 | 11 | 长时间任务不阻塞 |
| 7 | 权限分级模型 | 11 | 安全的自主执行 |
| 8 | Subagent 并行执行 | 13 | 复杂任务分解 |
| 9 | MCP 扩展集成 | 14+ | 第三方工具集成 |
